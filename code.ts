  ...
  for (var pi = 0; pi < figma.root.children.length; pi++) {
    ...
    for (var fi = 0; fi < flows.length; fi++) {
      ...
      allPrototypes.push({
        ...
        flowName:      flowName,
        type:          sceneNode.type,
        width:         Math.round((sceneNode as any).width || 0),
        height:        Math.round((sceneNode as any).height || 0),
        x:             Math.round((sceneNode as any).x || 0),
        y:             Math.round((sceneNode as any).y || 0),
        pageName:      page.name,
        pageId:        page.id,
        reactionsCount: ((sceneNode as any).reactions ? (sceneNode as any).reactions.length : 0) +
  ...
  switch (msg.type) {
    ...
    case "NAVIGATE_TO": {
      ...
      if (node && node.parent && node.parent.type === 'PAGE') {
        figma.currentPage = node.parent as PageNode;
